...
class ZegoLiveAudioRoomSeatManager with ZegoLiveSeatCoHost {
  ...
  bool isLeaveRoomDialogVisible=false;
...
////  getEmptySeats
  ...
  // leave room
  Future<bool> leaveRoom({bool showDialog = true}) async {
    ZegoLoggerService.logInfo(
      'leave room, showDialog: $showDialog',
      tag: 'audio room',
      subTag: 'room manager',
    );

    // Check if a confirmation dialog should be shown before leaving the room
    if (showDialog && isLeaveRoomDialogVisible) {
      ZegoLoggerService.logInfo(
        'leave room, dialog is already visible',
        tag: 'audio room',
        subTag: 'room manager',
      );
      return false;
    ...
    if (showDialog) {
      ...
      isLeaveRoomDialogVisible = true;
      final dialogInfo = innerText.leaveRoomDialogInfo; // Assuming you have dialog info set up
      ...
      await showLiveDialog(
        ...
        leftButtonCallback: () {
          ...
          Navigator.of(contextQuery!.call(), rootNavigator: config.rootNavigator).pop(false);
        },
        rightButtonText: dialogInfo.confirmButtonName,
        rightButtonCallback: () async {
          isLeaveRoomDialogVisible = false;

          // Proceed to actually leave the room
          await performLeaveRoom();

          // Close the dialog and return true (successful leave)
          Navigator.of(contextQuery!.call(), rootNavigator: config.rootNavigator).pop(true);

          // Pop the screen after leaving the room
      ...
      // If no dialog is shown, just leave the room and pop the screen
      await performLeaveRoom();
      Navigator.of(contextQuery!.call()).pop();
    }
  ...
  Future<void> performLeaveRoom() async {
    try {
      // Call Zego SDK to leave the room
