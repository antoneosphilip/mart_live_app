...
import 'package:live_app/view/screens/chat/widget/message_bubble.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<ChatController>(builder: (chatController) {
      ...
      String? baseUrl = '';
      if (widget.notificationBody!.receiverType == AppConstants.admin) {
        baseUrl = "";
        // Get.find<SplashController>().configModel!.baseUrls!.businessLogoUrl;
      } else {
        baseUrl = "${AppConstants.mediaUrl}/profile";
      }
        ...
        child: Scaffold(
          ...
          appBar: (AppBar(
            ...
            title: Text(chatController
                        .messageModel?.conversation?.receiverType ==
                    AppConstants.admin
                ? "migo"
                : widget.user != null
                ...
                child: Container(
                  ...
                  child: ClipOval(
                    ...
                    image: chatController
                                .messageModel?.conversation?.receiverType !=
                            AppConstants.admin
                        ? '$baseUrl/${widget.user != null ? "${widget.user?.image}" : "${chatController.messageModel?.conversation?.receiver?.image}"}'
                        : "",
                    fit: BoxFit.cover,
                    height: 40,
                    width: 40,
                                                ...
                                                itemBuilder: (context, index) {
                                                  return MessageBubble(
                                                    ...
                                                    userType: widget
                                                                .notificationBody !=
                                                            null
                                                        ? "${widget.notificationBody?.receiverType}"
                                                        : "${widget.notificationBody!.receiverType}",
                                                  );
                                                },
                                              ),
