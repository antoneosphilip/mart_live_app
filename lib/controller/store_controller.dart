...
import 'package:live_app/data/repository/store_repo.dart';
...
class StoreController extends GetxController
  ...
  Future<void> getTypes() async {
    loading = true;
    update();
    Response response = await storeRepo.getTypesList();
  ...
  Future<void> getItems(int? typeId) async {
    ...
    update();
    Response response =
        await storeRepo.getItemsList(selectedType?.id, currentPage);
    if (response.statusCode == 200) {
